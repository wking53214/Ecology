...
# ---------------------------------------------------------
@pytest.mark.xfail(
    reason="ClusterRunner() is no-arg (per test_cluster_runner_initialization) and "
           "nothing gives schedule() a telemetry reference -- no code path can populate "
           "'schedule' events for replay to reconstruct. See cluster_runner.py's module "
           "docstring. Companion test below characterizes the exact current gap shape.",
    strict=True,  # if this ever accidentally passes, FAIL loudly (XPASS) rather than
                  # silently -- an unnoticed fix is exactly the kind of drift worth catching
)
...
def test_cluster_simulator_integration_gap_is_characterized_not_fixed():
    """
    CHARACTERIZATION TEST, NOT A CORRECTNESS TEST. Passing here does NOT mean
    ClusterRunner/Simulator integration works. It means the known gap
    (ClusterRunner has no telemetry reference -- see cluster_runner.py's
    module docstring) still has EXACTLY the shape already documented, and
    hasn't silently drifted into a different broken state nobody's looked at.

test_cluster_simulator_integration directly above (marked xfail) is the
    permanent record of what CORRECT behavior would require. This test is
    the permanent record of what the CURRENT, known-incorrect behavior
    actually is, so a future change to either ClusterRunner or Simulator
    that alters the failure mode gets caught explicitly, instead of being
    silently absorbed into "yeah, that test's always been red, don't worry
    about it."

If this test starts failing: that does NOT mean "the gap got worse" by
    default. It means the gap CHANGED SHAPE and needs a fresh look, not an
    assumption that it's still the same known issue.
    """
    graph = build_graph()
    telemetry = TelemetryKernel()
    sim = Simulator(graph=graph, telemetry=telemetry, max_steps=10)
...
replay_cluster = ClusterRunner()
    for entry in telemetry.ledger:
        if entry["type"] == "schedule":
            replay_cluster.schedule(entry["payload"])

# Characterizes the gap precisely: cluster has 4 real, direct schedules.
    # replay has zero, because nothing ever gives schedule() a telemetry
    # reference to write "schedule" events into -- there is no code path
    # that could populate them as the classes are currently constructed.
    assert len(cluster.snapshot()["scheduled"]) == 4, \
        "cluster's own direct scheduling count changed -- investigate before assuming this is the known gap"
    assert replay_cluster.snapshot()["scheduled"] == [], \
        "replay reconstructed a schedule event from somewhere -- the gap changed shape, do not assume it's fixed without checking why"
