...
def test_monotonic_emotional_drift_under_real_friction_event():
    """
    Strengthens test_monotonic_emotional_drift, which passes on a bare
    DummyCallerDynamic by equality -- it can't tell the real event-driven
    logic from a no-op on that input (see LatentPayload finding #7/OQ6).
    This drives an actual friction_event through DynamicState and asserts
    STRICT movement, so a regression back to "nothing happens" would fail.
    """
    from domain.CallerState import DynamicState  # noqa: local import, avoids
    # breaking this file's import surface if CallerState isn't shimmed elsewhere
...
assert after["trust_scalar"] < before["trust_scalar"], "Trust did not strictly decrease on a real friction event."
    assert after["volatility"] > before["volatility"], "Volatility did not strictly increase on a real friction event."
    assert after["memory_flag"] > before["memory_flag"], "Memory did not strictly increase on a real friction event."
    assert cd.frustration > 0.0, "Frustration did not accrue on a real friction event."
    assert lp.friction_count == 1, "friction_count did not track the real event."
