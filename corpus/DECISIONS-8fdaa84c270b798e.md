...
## ADR-021 — Prescription confidence is a BAND with a method, never a bare number
**Context.** The prescription layer produces the number a CLIENT sees; the brief
calls a bare "+8%" a credibility bomb. **Decision.** Every prescription carries a
`ConfidenceAssessment` with per-fold estimates, the spread, seed sensitivity, a
categorical level, and a plain-language rationale — the point estimate never
travels alone. **Why.** A projected lift is only trustworthy if it is stable;
stating it as a range with the evidence is honest, a lone percentage is not.
**Status.** Locked. `prescription_types.py` / `classify_confidence`.

## ADR-022 — Confidence via hold-out-and-replicate (+ seed sensitivity)
**Context.** Need a real, non-theatrical confidence method. **Decision.** Split the
population into k folds, measure the change independently on each, and require sign
consistency to call it replicated; pair with re-running under different behavioral
seeds. The fold spread is the band. **Why.** This empirically answers "if you'd had
a fraction of the data, what range would you have gotten" — a genuine uncertainty.
Verified: it caught a naturally-occurring ghost ("lead with balance" = +0.4pt on
the full population but only 2/5 folds agree in sign) and refused it. We explicitly
do NOT claim formal statistical-CI rigor — the band is an empirical hold-out
spread, stated as such. **Status.** Locked.

## ADR-023 — The layer refuses: not-clean and ghost are structural rejections
**Context.** Honesty requires refusing bad recommendations, not just labeling them.
**Decision.** `PrescriptionStatus` is structural: REJECTED_NOT_CLEAN (simulator
verdict is false win / landmine / no improvement), REJECTED_GHOST (clean win that
fails replication), or CANDIDATE. A rejected change can never be mistaken for a
live one. A ghost is still RECORDED (not silently dropped) so the consultant knows
it was checked and found unstable. **Why.** The most seductive bad change is one
that looks like a win; structural refusal is stronger than a cautionary note.
**Status.** Locked.

## ADR-024 — Client-ready requires the Partner; the feedback loop records misses
**Context.** Two final honesty gates. **Decision.** (1) `client_ready` is true only
when a CANDIDATE also has validation=REAL and fairness=CLEAN from the Partner — the
prescription layer does its own replication check but never self-certifies for a
client. (2) `PrescriptionOutcome` records the projected band so a real post-
implementation actual can be scored HIT / PARTIAL / MISS, openly. **Why.** Sentinel
generates; the independent Partner signs off — two systems in tension. And the
discipline must survive being wrong in front of a client: recording misses is how
the tool earns trust and improves calibration across engagements. **Status.**
Locked by tests (not-client-ready-without-partner; feedback HIT/PARTIAL/MISS).
